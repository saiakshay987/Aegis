...
export default function AdminLogin() {
  ...
  return (
    <div className="min-h-screen hero-mesh relative overflow-hidden flex items-center justify-center p-4">
      ...
      >
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-violet-50 border border-violet-100 mb-4">
            <BankIcon className="w-7 h-7 text-violet-700" />
          </div>
          <h1 className="text-2xl font-bold text-slate-900">Bank Admin</h1>
          <p className="text-slate-500 text-sm mt-1">Portfolio Command Center</p>
        </div>

        <div className="mb-4">
          <label className="text-xs text-slate-500 uppercase tracking-wider mb-2 block">Admin Password</label>
        ...
        <button onClick={() => nav('/')}
          className="mt-6 text-xs text-slate-500 hover:text-slate-600 transition-colors w-full text-center">
          ← Back to home
        </button>
      </motion.div>
    </div>
